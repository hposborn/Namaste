...
def CalcDens(Star,nd=6000,returnpost=False,overwrite=True):
    ...
    if not hasattr(Star,'srads'):
        srads= GetAssymDist(Star.srad,Star.sraduerr,Star.sradderr,nd=nd)
    else:
        srads=Star.srads
    ...
    if hasattr(Star,'sdens') and not overwrite:
        ...
        if not hasattr(Star,'sloggs'):
            sloggs= GetAssymDist(Star.slogg,Star.slogguerr,Star.sloggderr,nd=nd)
        else:
            sloggs=Star.sloggs
        ...
        stdd1=np.inf
        if not hasattr(Star,'sloggs'):
            smasss= GetAssymDist(Star.smass,Star.smassuerr,Star.smassderr,nd=nd)
        else:
            smasss=Star.smasss
        masses[masses<0]=abs(np.random.normal(mass,0.25*(massuerr+massderr),np.sum(masses<0)))
